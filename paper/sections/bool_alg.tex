...
\begin{remark}[Homophonic Encryption]
If using homophonic encryption, identity is no longer maintained deterministically. A cipher set $\OT{\Set{A}}$ sent in one round may be \emph{incomparable} to the same set sent in another round.
\end{remark}
...
\end{theorem}
\begin{proof}
For membership testing, a false positive occurs when a non-member's hash bits are all contained within the set's bit vector. After inserting $n$ elements, each bit position has probability $1 - 2^{-n}$ of being set (assuming uniform hashing). A query element maps to $m$ bit positions, all of which must be set for a false positive. By independence, $\fprate = (1 - 2^{-n})^m \approx \alpha^m(n)$.
\end{proof}
...
\end{corollary}
\begin{proof}
From the false positive rate theorem, $\fprate = \alpha^m(n)$, so $m = \log_\alpha(\fprate) = \log_2(\fprate)/\log_2(\alpha)$. The bit-rate is $m/n$, yielding $\Fun{b} = \log_2(\fprate)/(n \cdot \log_2(\alpha)) \approx \log_2(\fprate)/(n \cdot \alpha(n))$ for the approximation $\log_2(\alpha) \approx \alpha$ when $\alpha$ is close to 1.
\end{proof}
...
\begin{theorem}
	False positives on the \emph{equality} predicate occur with probability
	\begin{equation}
	\fprate(m) = 2^{-m}\,,
	\end{equation}
...
\end{theorem}
\begin{proof}
	Two sets are said to be identical if they map to the same bit string. A false positive occurs when $\Fun{G}(\Set{X}) = \Fun{G}(\Set{Y})$ but $\Set{X} \neq \Set{Y}$. Under the random oracle assumption, the probability that two distinct inputs produce the same $m$-bit output is $2^{-m}$ (the birthday bound gives a higher collision probability for many comparisons, but for a single pair comparison, it is $2^{-m}$).
\end{proof}

\begin{remark}[Subset Testing]
For subset testing $\Set{A} \subseteq \Set{B}$, a false positive occurs when all bits of $\Fun{G}(\Set{A})$ are contained in $\Fun{G}(\Set{B})$ despite $\Set{A} \not\subseteq \Set{B}$. The false positive rate depends on the relative sizes and overlap of the sets.
\end{remark}
