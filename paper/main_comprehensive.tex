...
The error behavior of an approximate Boolean is captured by its confusion matrix:
$$\ConfMat = \begin{pmatrix} 1-\beta & \beta \\ \alpha & 1-\alpha \end{pmatrix}$$
where entry $\ConfMat_{ij}$ gives the probability of observing $j$ given latent value $i$. Perfect observation corresponds to the identity matrix; complete information loss corresponds to a rank-1 matrix (identical rows).
\end{definition}
...
All HBOS operations derive from a general hash-based construction that unifies probabilistic data structures with cryptographic privacy:

\begin{definition}[Hash-Based Bernoulli Type]
A hash-based Bernoulli type consists of:
\begin{enumerate}
...
The false positive rate emerges from encoding set sizes: $\alpha = |\ValidEnc{\mathsf{true}}|/2^m$. This unified view reveals that Bloom filters, trapdoor functions, and approximate maps are all instances of the same construction with different encoding strategies.

\subsection{Privacy-Space Trade-off}

The same hash construction achieves fundamentally different goals by varying encoding sizes:
...
\textbf{Privacy-Optimal Encoding}: $|\ValidEnc{y}| \propto 1/\text{freq}(y)$ (inverse frequency). Achieves uniform output distribution, hiding frequency patterns. This is the \emph{1/p(y) principle}---the key insight connecting approximation to privacy.
...
HBOS uses uniform-sized encodings (a middle ground), providing practical privacy at constant space cost per element.
